...
const createOrder = (order) => async (dispatch, getState) => {
  try {
    ...
    console.log(order)
    const order_i = {"author":userInfo.user.id,
    "itemsPrice":order.itemsPrice,
    "freightPrice":order.freightPrice,
    "totalPrice":order.totalPrice,
    "orderItems":order.orderItems}

    const { data: { data: newOrder } } = await Axios.post("http://localhost:8000/order/", order_i, {
      headers: {
...
const listMyOrders = () => async (dispatch, getState) => {
  try {
    ...
    const { userSignin: { userInfo } } = getState();
    const { data } = await Axios.get("http://localhost:8000/list-my-orders/" + userInfo.user.id, {
      headers:
        { Authorization: 'Token ' + userInfo.token }
    });
...
const detailsOrder = (orderId) => async (dispatch, getState) => {
  try {
    ...
    const { userSignin: { userInfo } } = getState();
    const { data } = await Axios.get("http://localhost:8000/order" + orderId, {
      headers:
        { Authorization: 'Token ' + userInfo.token }
    });
